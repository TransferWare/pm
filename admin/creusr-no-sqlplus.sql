...
declare
  -- Cloud defaults
  c_pm_owner_username constant all_users.username%type := 'PM_OWNER';
  c_tablespace_users constant user_tablespaces.tablespace_name%type := 'DATA';
  c_tablespace_temp constant user_tablespaces.tablespace_name%type := 'TEMP';

  -- ORA-01920: user name 'PM_OWNER' conflicts with another user or role name
  e_user_already_exists exception;
  pragma exception_init(e_user_already_exists, -1920);
begin
  execute immediate utl_lms.format_message
                    ( 'create user %s no authentication default tablespace %s temporary tablespace %s'
                    , c_pm_owner_username
                    , c_tablespace_users
                    , c_tablespace_temp
                    );
  ...
  execute immediate utl_lms.format_message
                    ( 'grant select_catalog_role to %s'
                    , c_pm_owner_username
                    );
exception
